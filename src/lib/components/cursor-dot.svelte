<script lang="ts">
  ...
  function updateMouseCoords(e: MouseEvent) {
    ...
    if (cursor === "pointer") {
      let currentlyHovering: Element = document.elementFromPoint(
        e.clientX,
        e.clientY
      )!;
  ...
  function animate(node: HTMLElement) {
    ...
    currentPosition.x +=
      easeInOutQuad(t) * (targetPosition.x - currentPosition.x);
    currentPosition.y +=
      easeInOutQuad(t) * (targetPosition.y - currentPosition.y);
