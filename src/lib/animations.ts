...
export function letterSlideIn(
  ...
  return {
    ...
    tick: (t: number) => {
      let eased = BezierEasing(0.2, 0.58, 0.43, 1)(t); // t value with easing applied

      masks.forEach((element: HTMLElement) => {
        // Letter animation
        element.childNodes.forEach((e) => {
          (e as HTMLElement).style.transform = `translate3d(${(
            150 +
            -eased * 150
          ).toFixed(2)}%, 0px, 0px)`;
        });
        ...
        // Word animation
        element.style.transform = `translate3d(${(80 + -eased * 80).toFixed(
          2
        )}%, 0px, 0px)`;
      });
  ...
  // Call animation programmatically outside of svelte transitions with anime.js
  function animeAnimation(animeParams?: {
    easing?: string;
    onComplete?: () => void;
  }) {
    ...
    anime({
      ...
      complete: () => {
        ...
        node.innerHTML = originalNodeHTML;
        if (animeParams) if (animeParams.onComplete) animeParams.onComplete();
      },
    });
  }
}
...
export function letterSlideOut(
  node: HTMLElement,
  params?: {
    duration?: number;
    delay?: number;
  ...
  return {
    ...
    // Svelte transition animation
    tick: (t: number) => {
      let eased = BezierEasing(0.32, 0.24, 0.76, 0.26)(t);

      masks.forEach((element: HTMLElement) => {
        element.childNodes.forEach((child) => {
          (child as HTMLElement).style.transform = `translate3d(${(
            -150 +
            eased * 150
          ).toFixed(2)}%, 0px, 0px)`;
        });
...
export function maskSlideIn(
  ...
  return {
    ...
    tick: (t: number) => {
      ...
      let eased = BezierEasing(0.58, 0.14, 0.06, 0.97)(t);

      if (params!.reverse) {
        mask.style.transform = `translate3d(${(100 + -eased * 100).toFixed(
          2
        )}%, 0px, 0px)`;
        node.style.transform = `translate3d(${(-100 + eased * 100).toFixed(
          2
        )}%, 0px, 0px)`;
      } else {
        mask.style.transform = `translate3d(${(-100 + eased * 100).toFixed(
...
export function maskSlideOut(
  ...
  return {
    ...
    tick: (t: number) => {
      ...
      if (isParentMask) {
        node.parentElement!.style.transform = `translate3d(${(
          -100 +
          eased * 100
        ).toFixed(2)}%, 0px, 0px)`;
      }
...
export function workImageIntro(
  node: HTMLElement,
  params?: { promise: Promise<any>; delay?: number }
) {
...
// Animation for workItem image when workContainer is scrolled into view
export function workListIntro(
  node: HTMLElement,
  params?: { promise: Promise<any>; delay?: number; onComplete?: () => void }
) {
  if (params === undefined) return;
...
function tagLettersAndWords(node: HTMLElement, params: { breakWord: boolean }) {
  ...
  if (masks.length < 1) {
    node.innerHTML = parseLetters(
      node.innerHTML,
      '<div class="a-text-mask"><div class="a-text-block">',
      "</div></div>"
    );
    ...
    [...string].forEach((e, i) => {
      ...
      if (isTag) {
        ...
        if (
          ...
          newString += isWord
            ? '<div class="a-word">'
            : '</div><span class="a-spacer a-text-block"> </span>';
        }
